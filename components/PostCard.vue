...
<template>
  <nuxt-link ref="card" :to="post.path" class="block max-w-full transition-all group">
    <div class="flex items-center">
      <div class="opacity-70 group-hover:(opacity-90) transition-all absolute top-3 -left-20 post-time">
        <div class="text-lg">
          {{ month }}
        </div>
        <div class="text-sm">
      ...
      <div class="max-w-full">
        <h3 class="opacity-90 text-1xl mb-3 group-hover:(sm:-mx-3 tracking-wide text-blue-700 font-bold) transition-opacity post-title">
          <span>{{ post.title }}</span>
        </h3>
        <div class="text-sm wrap overflow-hidden whitespace-nowrap mb-3 flex items-center post-description">
